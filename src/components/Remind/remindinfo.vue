...
<div class="mui-content">
			<div class="mui-content-padded" style="margin: 5px;text-align: center;">
				...
				<button id='confirmBtn' type="button" class="mui-btn mui-btn-blue mui-btn-outlined" @click="AddRemind">添加提醒</button>
        ...
        position(){  // 设置位置 以及绑定事件
        	...
        	AddRemind() {
				...
				mui.confirm('添加此时间提醒，确认？', '添加', btnArray, (e)=> {
					if (e.index == 1) {
						info.innerText = '添加成功,时间为：'+this.year+'年'+this.month+'月'+this.currentDay+'日 ';
					} else {
						info.innerText = '已取消'
					}
				})
...
<style scoped lang="scss">
    ...
    .remindinfo{
        ...
        h2{ 
            ...
            letter-spacing: 8px;
            padding: 5px;
            background-color:rgb(251, 126, 113) ;
           color: #fff;
        }       
        img{
            width: 60px;
